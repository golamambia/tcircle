...
const UnreadMessage = ({ navigation }) => {
 ...
 useEffect(()=>{
       const SAPCode=getUserName ? getUserName : userName;
      ...
      const payload={
             ...
             "ToDate" : '',
              "Customer_Id"  : getUserName ? getUserName : userName
                ...
                console.log(payload);
                setLoading(true);
                CustomerService.getUnreadMessageList(payload)
                              .then(res => {
                                const result=res.COALBPSAppGetUnReadNotificationCountResult;
                                console.log('COALBPSAppGetUnReadNotificationCountResult res', JSON.stringify(res));
                                setLoading(false);
                                if (result.StatusCode=='001'  && result.Message=='Success') {
                                  if (result.notificationList && result.notificationList.length > 0) {
                                    const hasValidData = result.notificationList.some(
                                      (item) =>
  ...
  return (
    <InsideFormBg paddingHorizontal={false} headerText="Unread Message" back={true} refreshIcon={true} onRefreshIcon={()=>searchHandler()}>
